...
public class Queries {
	public static void enter_new_publication(User p) {
		...
		try{
			p.in.nextLine();
			System.out.println("Enter following details: %n 1) publication_id %n 2) title %n 3) typical_topics %n 4) type %n 5) periodicity %n %n");
			...
			s1 = (PreparedStatement) p.conn.prepareStatement("INSERT INTO Publication (publication_id, title, typical_topics, type, periodicity) VALUES (?,?,?,?,?) ");
	...
	public static void update_publication_info(User p){
		...
		try{
			...
			System.out.println("What do you want to update? %n 1) title %n 2) typical_topics %n 3) type %n 4) periodicity %n %n");
			...
			if (ch == 1) {
				...
				s2 = (PreparedStatement) p.conn.prepareStatement("UPDATE Publication SET title = ? WHERE publication_id = ?"); 
				...
				s2 = (PreparedStatement) p.conn.prepareStatement("UPDATE Publication SET typical_topics = ? WHERE publication_id = ?"); 
				...
				s2 = (PreparedStatement) p.conn.prepareStatement("UPDATE Publication SET type = ? WHERE publication_id = ?"); 
				...
				s2 = (PreparedStatement) p.conn.prepareStatement("UPDATE Publication SET periodicity = ? WHERE publication_id = ?"); 
